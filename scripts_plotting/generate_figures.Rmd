...
p_violin <- ggplot(df,
  ...
  geom_violin(fill=NA,alpha=1,draw_quantiles = c(0.25,0.5,0.75),size=0.5)+
  scale_y_log10nice()+
...
p_TPM_TE <- ggplot(df,
  ...
  geom_point(alpha=0.2, size = 1.5, shape = 16, color = "black", )+
  geom_point(data=df %>% filter(gene%in%genes_to_label),size=2,color="red")+
...
p_Z_box <- ggplot(df,
                  aes(x=1,y=Zsup.mean))+
  geom_hline(yintercept=0,linetype="dotted")+
  geom_point(data=df%>%filter(gene%!in%genes_to_label),position=position_jitter(width=0.43),alpha=0.01)+
...
p_zsup_46C <- ggplot(df,
  ...
  labs(x="",y="")
p_zsup_46C
ggsave("../figures/Supplemental/Zsup_30vs46.png",
...
p_zsup_boxes <- ggplot(df,
  ...
  geom_hline(yintercept=0,linetype="dotted")+
    geom_point(position=position_jitterdodge(jitter.width=1.5),
               aes(y=deltaZ.trimmed,color=target),shape=16,alpha=0.3)+
  scale_color_manual(values=target.cols)+
  geom_violin(fill=NA,draw_quantiles = c(0.25,0.5,0.75),,size=0.25,color="black")+
    theme(legend.position="none")+
...
p_zsup_boxes
ggsave("../figures/Supplemental/Zsup_hsf1_boxplot.png",
...
p_spec <- ggplot(df,
  ...
  geom_point(aes(group=interaction(Stress_label,stress_specific),alpha=stress_specific),
                 shape=16,color="black",position=position_jitterdodge(jitter.width = 0.2,
                                                                                dodge.width = 0.75))+
  scale_alpha_manual(values=c("none"=0.02,"42C"=0.3,"Azide 0.8%"=0.3,"Ethanol 7.5%"=0.3))+
  ...
  theme(legend.position = "none")
p_spec
ggsave("../figures/Supplemental/specific_boxplots.pdf",p_spec,width=100,height=50,units="mm",bg="transparent")
```
...
ggsave("../figures/Supplemental/length_dep_HS.png",
...
ggsave("../figures/Supplemental/length_dep_N3.png",
...
ggsave("../figures/Supplemental/length_dep_EtOH.png",
...
ggsave("../figures/Supplemental/Muhlhofer_upr.png",
...
df_hairpins <- df_Zsup_mean %>% filter(Treatment_group=="hairpinReporters") %>%
  group_by(ORF) %>%
  summarise(SP.mean = exp(mean(log(SP))),
            Zsup.mean = mean(Zsup),
            TPM.mean = exp(mean(log(Total.TPM))))
df_30C_occ <- df_Occ_mean_minfilt %>%
  filter(Treatment%in%c("none","mock"),Temperature=="30C") %>%
  group_by(ORF) %>%
  summarise(Occ.odds.mean = exp(mean(log(Occ.odds.mean))))
df <- df_hairpins %>% full_join(df_30C_occ,by="ORF") %>%
  ...
  left_join(df_Weinberg,by="ORF") %>%
  filter(Occ.odds.mean>0) %>%
  left_join(df_rnafold %>% filter(Target=="first50UTR5") %>%
              select(ORF,Target,Energy,EnergyBin),by="ORF") %>%
  mutate(gene_label = if_else(gene%in%genes_to_label,gene,""))
df_corr <- df %>% ungroup %>%
  nest() %>%
  mutate(corr = map(data,function(df){
...
p_3B_occ
ggsave("../figures/Supplemental/HAC1_GCN4_Occupancy.png",egg::set_panel_size(p=p_TPM_Occ+labs(x="",y=""),width=unit(30,"mm"),height=unit(30,"mm")),
