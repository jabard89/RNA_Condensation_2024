...
native_stats <- df %>%
  ...
  nest() %>%
  mutate(p.value = map_dbl(data, ~ wilcox.test(log10(SP.FC) ~ Probe, data = .x)$p.value),
         label = case_when(
           p.value < 0.001 ~ "***",
           p.value < 0.01 ~ "**",
           p.value < 0.05 ~ "*",
...
synth_stats <- df_combined_plot %>%
  filter(Group != "native") %>%
  group_by(UTR5, Temperature) %>%
  nest() %>%
  mutate(p.value = map_dbl(data, ~ t.test(log10(SP.FC) ~ Reporter_age, data = .x)$p.value),
         label = case_when(
           p.value < 0.001 ~ "***",
           p.value < 0.01 ~ "**",
           p.value < 0.05 ~ "*",
...
p_chx <- ggplot(df,
  ...
  # ggpattern::geom_boxplot_pattern(aes(pattern=Treatment),alpha=0.8,outlier.shape=NA,
  #                                pattern_density=0.5,
  #                                pattern_spacing=0.035,
  #                                pattern_fill="white",
  #                                pattern_size=0.1)+
  #ggpattern::scale_pattern_manual(values=c("mock"="none","CHX"="stripe"))+
  #geom_violin(fill=NA,draw_quantiles = c(0.5),color="black")+
  scale_fill_manual(values=stress.cols)+
  labs(x="",y="")+
  guides(fill="none",pattern="none")+
  scale_x_discrete(labels=c("30C"="30\u00B0C", "42C"="42\u00B0C", "46C"="46\u00B0C"))+
  #scale_y_continuous(labels=c(0,0.2,0.5,0.8,1))
...
p_chx_sed_bytreat <- ggplot(df_42_46,
  ...
  scale_x_discrete(labels = c("30C" = "30\u00B0C", "42C" = "42\u00B0C", "46C" = "46\u00B0C")) +
  scale_y_reverse(
    breaks = c(10, 5, 0),    # Explicitly setting the breaks to 10, 5, 0
    limits = c(13, -3),       # Reversing the y-axis with limits from 10 to 0
    oob = scales::oob_keep   # Ensure data outside the limits is still visible
  ) +
  scale_alpha_manual(values = c(0.9, 0.9)) +
  guides(alpha = "none", linetype = "none") +
  theme(
...
p_chx_sed_compMock <- ggplot(df,
  ...
  scale_x_discrete(labels = c("30C" = "30\u00B0C", "42C" = "42\u00B0C", "46C" = "46\u00B0C")) +
  scale_y_reverse(
    breaks = c(0, 5, 10),  # The breaks for your reversed y-axis
    limits = c(12, -2),    # The reversed limits: 13 at the bottom and -3 at the top
    oob = scales::oob_keep  # Ensure data outside the limits is still visible
  ) +
  scale_alpha_manual(values = c(0.9, 0.9)) +
  guides(alpha = "none", linetype = "none") +
  theme(
    axis.title.x = element_text(),
    axis.title.y = element_text(),
    axis.text.y = element_text(),  # Ensure y-axis labels are visible
    axis.ticks.y = element_line())
...
p_3b_violin_sed <- ggplot(df %>% filter(Treatment_group == "eIF3b_deplete"),
  ...
  scale_y_reverse(
    breaks = c(0, 5, 10),  # The breaks for your reversed y-axis
    limits = c(13, -3),    # The reversed limits: 13 at the bottom and -3 at the top
    oob = scales::oob_keep  # Ensure data outside the limits is still visible
  ) +
  scale_alpha_manual(values = c(0.9, 0.9)) +
  guides(alpha = "none", linetype = "none") +
