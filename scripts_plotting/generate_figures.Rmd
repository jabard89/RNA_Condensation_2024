...
gsed = ggplot(res, aes(x=length, y=sed.mean, fill=cat)) + theme(plot.margin=unit(c(1,0,1,1),'cm')) +
  ...
  scale_y_reverse("Differential sedimentation (SDs)", limits=c(7,-0.5)) + no.legend
...
#psw_hs = dat_hs |> select(ORF, gene, label, length, Temperature, pSup.mean, esc.mean, sed.mean) |> pivot_wider(id_cols=c(ORF,gene,label,length), values_from=c(pSup.mean, esc.mean, sed.mean), names_from=Temperature) #, names_prefix = c("pSup.")
dat_azide = df_Zsup_mean |> filter(Treatment_group=='azide3') |> mutate(length=LengthTxEst)
pskho = dat_azide |> filter(Treatment=='Azide_0.8%_pH6.8') |> select(ORF, length, pSup.mean, sed.mean, esc.mean) |> left_join(kho |> select(-gene, -length), by='ORF') # |> mutate(deltaSed=zSup.azide-zSup.mock)
...
#pskho_hs = psw |> left_join(kho |> select(-gene, -length), by='ORF')

#pskho = psw_hs |> left_join(kho |> select(-gene), by='ORF')
#pskho_sub = na.omit(pskho |> select(ORF, gene, pSup.46C, foldchange.sg.total, sg.classification) |> filter(pSup.46C>0 & #foldchange.sg.total>0))
#ct = cor.test(logodds(pskho_sub$pSup.46C), log(pskho_sub$foldchange.sg.total), method='s')

# gkhong_hs = ggplot(pskho_hs, aes(x=foldchange.sg.total, y=sed.mean, colour=sg.classification)) + 
#   geom_point(shape=shap, alpha=alph) + 
#   scale_y_continuous("Differential sedimentation (&#394sed)") + 
...
p_khong_vs_pSup_mock = ggplot(pskho_mock, aes(x=foldchange.sg.total, y=1-pSup.mean, colour=sg.classification)) + 
  geom_point(shape=shap, alpha=alph) + 
  scale_y_log2nice("Proportion in pellet (1-pSup),\nmock treatment", limits=2^c(-4.5,0)) + 
  scale_x_log2nice("Stress granule enrichment (Khong *et al.*)", limits=c(0.125,8)) +
  scale_colour_manual(values=sg_enrichment_colors) +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) + no.legend + larger_labels +
  theme(axis.title.x = ggtext::element_markdown())
...
#cor.test(exp(pskho_hs$sed.mean), pskho_hs$foldchange.sg.total, method='s')
...
print(p_khong)
#save_plot("../figures/khong-comparison.pdf", pg, base_asp=2, base_height=4)
...
```{r}
num_most_induced_genes <- 50 #number of most induced genes to use in analysis

df_azide_08_delta <- df_Zsup_mean %>% filter(Treatment_group %in% c('azide3')) %>%
  select(ORF,gene, LengthTxEst,Strain,Treatment_group,Treatment, Temperature, Total.TPM.mean,Zsup.mean) %>%
  pivot_wider(names_from = c(Treatment,Temperature), values_from = c(Total.TPM.mean,Zsup.mean)) %>%
...
# #import 0.8%
# df_azide_08_delta <- df_data %>% filter(Treatment_group %in% c('azide3')) %>%
#   select(-Treated,-SP.mean,-pSup.mean) %>%
#   pivot_wider(names_from = Treatment, values_from = c(Total.TPM.mean,Zsup.mean)) %>%
#   mutate(tpm_fc_azide_08 = as.numeric(`Total.TPM.mean_Azide_0.8%_pH6.8`) / as.numeric(Total.TPM.mean_mock),
#          Zsup_delta_azide_08 = as.numeric(`Zsup.mean_Azide_0.8%_pH6.8`) - as.numeric(Zsup.mean_mock))
# #combine
# df_azide_cum <- rbind(df_azide_05_delta %>%
#                         select(ORF,LengthTxEst,tpm_fc_azide_05,Zsup_delta_azide_05) %>%
#                         mutate(Treatment = 'Azide_05') %>%
#                         dplyr::rename(tpm_fc = tpm_fc_azide_05, Zsup_delta = Zsup_delta_azide_05),
#                       df_azide_08_delta %>%
#                         select(ORF,LengthTxEst,tpm_fc_azide_08,Zsup_delta_azide_08) %>%
#                         mutate(Treatment = 'Azide_08') %>%
#                         dplyr::rename(tpm_fc = tpm_fc_azide_08, Zsup_delta = Zsup_delta_azide_08))
#ethanol
#import 7.5%
...
# #import 10%
# df_etoh_10_delta <- df_data %>% filter(Treatment_group %in% c('milderEtOH1') & !(Treatment %in% c('EtOH_5%'))) %>%
#   select(-Treated,-SP.mean,-pSup.mean) %>%
#   pivot_wider(names_from = Treatment, values_from = c(Total.TPM.mean,Zsup.mean)) %>%
#   mutate(tpm_fc_ethanol_10 = as.numeric(`Total.TPM.mean_EtOH_10%`) / as.numeric(Total.TPM.mean_mock),
#          Zsup_delta_ethanol_10 = as.numeric(`Zsup.mean_EtOH_10%`) - as.numeric(Zsup.mean_mock))
# #combine
# df_ethanol_cum <- rbind(df_etoh_75_delta %>%
#                         select(ORF,LengthTxEst,tpm_fc_ethanol_75,Zsup_delta_ethanol_75) %>%
#                         mutate(Treatment = 'Ethanol_7.5') %>%
#                         dplyr::rename(tpm_fc = tpm_fc_ethanol_75, Zsup_delta = Zsup_delta_ethanol_75),
#                       df_etoh_10_delta %>%
#                         select(ORF,LengthTxEst,tpm_fc_ethanol_10,Zsup_delta_ethanol_10) %>%
#                         mutate(Treatment = 'Ethanol_10') %>%
#                         dplyr::rename(tpm_fc = tpm_fc_ethanol_10, Zsup_delta = Zsup_delta_ethanol_10))
...
# df_temp_46_delta <- df_data %>% filter(Treatment_group %in% c('EW_TSPP') & Temperature %in% c('30C','46C')) %>%
#   select(-Treated,-SP.mean,-pSup.mean) %>%
#   pivot_wider(names_from = Temperature, values_from = c(Total.TPM.mean,Zsup.mean)) %>%
#   mutate(tpm_fc_temp_46 = as.numeric(Total.TPM.mean_46C) / as.numeric(Total.TPM.mean_30C),
#          Zsup_delta_temp_46 = as.numeric(Zsup.mean_46C) - as.numeric(Zsup.mean_30C))
# #combine
# df_temp_cum <- rbind(df_temp_42_delta %>%
#                         select(ORF,LengthTxEst,tpm_fc_temp_42,Zsup_delta_temp_42) %>%
#                         mutate(Treatment = 'Temp_42C') %>%
#                         dplyr::rename(tpm_fc = tpm_fc_temp_42, Zsup_delta = Zsup_delta_temp_42),
#                       df_temp_46_delta %>%
#                         select(ORF,LengthTxEst,tpm_fc_temp_46,Zsup_delta_temp_46) %>%
#                         mutate(Treatment = 'Temp_46C') %>%
#                         dplyr::rename(tpm_fc = tpm_fc_temp_46, Zsup_delta = Zsup_delta_temp_46))
...
df_etoh_75_delta <- df_etoh_75_delta %>%
  select(ORF,LengthTxEst,tpm_fc_etoh_75,Zsup_delta_etoh_75) %>%
  mutate(Treatment = 'Ethanol_75') %>%
  dplyr::rename(tpm_fc = tpm_fc_etoh_75, Zsup_delta = Zsup_delta_etoh_75)
...
df_combineStress_wide <- df_combineStress_cum %>% pivot_wider(data = ., names_from = c(Treatment), values_from = c(tpm_fc,Zsup_delta))
...
df_heatEtOHStress_long <- df_heatEtOHStress_wide %>%
  select(ORF,Zsup_delta_Azide_08,Zsup_delta_Ethanol_75,Zsup_delta_Temp_42C,inducedCategory) %>%
  pivot_longer(., cols = c("Zsup_delta_Azide_08","Zsup_delta_Ethanol_75","Zsup_delta_Temp_42C"), names_to = "Stress", values_to = "Zsup_delta")

df_heatEtOHStress_long$Stress[which(df_heatEtOHStress_long$Stress == "Zsup_delta_Azide_08")] <- "0.8% Azide"
df_heatEtOHStress_long$Stress[which(df_heatEtOHStress_long$Stress == "Zsup_delta_Temp_42C")] <- "42°C"
df_heatEtOHStress_long$Stress[which(df_heatEtOHStress_long$Stress == "Zsup_delta_Ethanol_75")] <- "7.5% Ethanol"
...
p_stressSpecificEscape_heatEtOH2 <- ggplot(data = df_heatEtOHStress_long %>% filter(Stress %in% c('42°C','7.5% Ethanol')),
       aes(y = Zsup_delta, x = inducedCategory, fill = inducedCategory)) +
  ...
  scale_x_discrete(name = 'Induction Category',
                   labels = c('other' = 'Neither',
                              ...
                              'etoh,azide' = 'Ethanol Azide',
                              'temp,etoh,azide' = 'Temp Ethanol Azide')) +
  scale_y_continuous(name = '∆Zsup (stress)',
                     limits = c(-4,4)) +
  scale_fill_manual(values = c('grey','orange','#A155B9','purple')) +
  facet_grid(cols = vars(Stress)) +
  theme(legend.position = 'none')
p_stressSpecificEscape_heatEtOH2

ggsave(file.path(github_dir,"figures","Figure3/stressSpecificEscape_heatEtoh.pdf"),
       p_stressSpecificEscape_heatEtOH2,
       width=150,height=100,units="mm",bg="transparent", device = cairo_pdf)
```
...
p_RelSed_box <- ggplot(df,
  ...
  ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(gene%!in%genes_to_label),position=position_jitter(width=0.43),alpha=0.03, size = 0.09)) +
  #geom_boxplot(fill=NA,outlier.shape=NA)+
  geom_violin(fill="grey70", alpha = 0.6, draw_quantiles = c(0.5))+
  ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(gene%in%genes_to_label),alpha=1,color="red",size=1)) +
  #geom_text_repel(data=df%>%filter(gene=="GCN4"),size=4.2,aes(label=gene_label),max.overlaps = 10000,min.segment.length = 0,nudge_y=2.5,nudge_x=0.25)+
...
ggsave(file.path(github_dir,"figures","Figure5","DTT_esc.pdf"),
                 p_dtt+labs(x="",y=""),
       width=42.5,height=42.5,units="mm",bg="transparent")
```
...
```{r}
df_Zsup_mean <- read_tsv(file.path(github_dir, "data_processed/sedseq_filt_mean_lo.tsv.gz")) |>
  left_join(df_samples_byLysate |> 
              select(Treatment_group,Treatment,Temperature,Time) |> unique(),
              by=c("Treatment","Treatment_group","Temperature")) |>
  left_join(gene_labels |> select(-LengthTxEst), by='ORF')
df <- df_Zsup_mean %>%
  ...
  summarise(pSup.mean = invlogodds(mean(logodds(pSup.mean))),
            SP.mean.all = exp(mean(log(SP.mean))),
            SP.sem.all  = exp(sd(log(SP.mean))/sqrt(n())),
            lo.pSup.windowmean = mean(lo.pSup.treatment.window.mean.mean),
            odds.pSup.windowmean = exp(lo.pSup.windowmean),
            RelSed.mean = mean(RelSed.mean)) %>%
  ungroup %>%
  mutate(lo.pSup.hairpin.sd = sd(logodds(pSup.mean) - lo.pSup.windowmean, na.rm=T))
# not bad set.seed(1)
set.seed(1)
which_genes <- df_Zsup_mean %>%
  filter(Treatment_group == "hairpinReporters",
...
set_label_size = theme(
  ...
  axis.title.y = element_text(size = 10),
  axis.text.y = element_text(size = 10))
p_rep <- ggplot(df_Zsup_mean %>% filter(Treatment_group == "hairpinReporters"),
            aes(x=LengthTxEst,y=SP.mean))+
  ggrastr::rasterize(dpi = 600, geom_point(alpha=0.3,shape=16,size=0.5,color="grey60")) +
  geom_line(aes(y=exp(lo.pSup.treatment.window.mean.mean)),color="black",size=0.5,alpha=0.5,linetype=1,linewidth=0.5) +
...
p <- ggplot(df,
  ...
  ggrastr::rasterize(dpi = 600, geom_point(alpha=0.3,shape=16,size=0.5,color="grey60")) +
  geom_line(aes(y=odds.pSup.windowmean),color="black",size=0.5,alpha=0.5,linetype=1,linewidth=0.5)+
  geom_line(aes(y=(odds.pSup.windowmean-exp(lo.pSup.hairpin.sd))),color=okabe_ito_colors[4],size=1,linewidth=0.5, linetype=1)+
  geom_line(aes(y=(odds.pSup.windowmean+exp(lo.pSup.hairpin.sd))),color=okabe_ito_colors[4],size=1, linewidth=0.5, linetype=1)+
  scale_x_log10nice()+ scale_y_log10nice()+
  coord_cartesian(ylim=c(0.5,40))+
  geom_point(data=df %>% filter(gene %in% c(genes_to_highlight)),
...
ggsave("../figures/Supplemental/hairpin_relsed_30C_error2.pdf",
        egg::set_panel_size(p=p,
                            width=unit(50,"mm"),height=unit(50,"mm")),
       width=75,height=75,units="mm",bg = "transparent",dpi=600)
```
...
p_TPM_Occ <- ggplot(df,
  ...
  ggrastr::rasterize(dpi = 600, geom_point(alpha=0.2, size = 0.3, shape = 16)) +
  geom_point(data=df %>% filter(gene%in%genes_to_label),size=2,color="red")+
  geom_text_repel(data=df,size=4.2,aes(label=gene_label),max.overlaps = 10000,min.segment.length = 0)+
...
p_3B_occ
ggsave("../figures/Supplemental/HAC1_GCN4_Occupancy.pdf",egg::set_panel_size(p=p_TPM_Occ+labs(x="",y=""),width=unit(50,"mm"),height=unit(50,"mm")),bg="transparent",dpi=600)
```
