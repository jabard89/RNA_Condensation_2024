...
```{r fig1_psup_vs_length, dependson="setup"}
...
g_data = ggplot(df, aes(x=length, y=pSup.mean, group=Temperature, colour=Temperature)) + 
  ...
  geom_line(aes(y=pSup.treatment.window.mean.mean, group=Temperature), linewidth=1, linetype=1, colour=blackcol, alpha=0.5) +
  ...
  #geom_vline(xintercept = 3600, linetype='11', alpha=0.5) +
  coord_cartesian(ylim=c(0,1)) +
...
g_trans = ggplot(df, aes(x=length, y=trans(pSup.mean), group=Temperature, colour=Temperature)) + 
  ...
  geom_line(aes(y=trans(pSup.treatment.window.mean.mean), group=Temperature), linewidth=1, linetype=1, colour=blackcol, alpha=0.7) +
  scale_colour_manual(values=stress.cols) + no.legend +
...
# # Introducing delta-zSup
# g_zsup = ggplot(dh, aes(x=length,y=Zsup,color=Temperature)) +
#   geom_hline(yintercept=0,linetype="dotted",color="grey50") +
#   geom_point(alpha=0.2,shape=16, size = 1) +
#   scale_x_log10nice("mRNA length (nucleotides)") +
#   facet_grid(.~Temperature) +
#   scale_y_continuous("Sedimentation score (SDs)",breaks=seq(-4,4)) +
#   coord_cartesian(ylim=c(-4.5,4.5)) +
#   no.legend +
#   scale_color_manual(values=stress.cols)+
#   geom_point(data=dh |> filter(gene%in%focal_genes),fill=NA,alpha=1,size=4,shape=16,stroke=1,color="black")+
#   geom_point(data=dh |> filter(gene%in%focal_genes),aes(fill=Temperature),alpha=1,size=2,shape=16,stroke=1)+
#   scale_fill_manual(values=stress.cols)+
#   theme(strip.text.x = element_blank(),
#         panel.spacing.x = grid::unit(0, "mm"),
#         panel.border = element_rect(colour = "black", fill = NA, size = 0.35) # Adjust size for thickness
#         )  + larger_labels
...
res = res_hs #bind_rows(res_hs10, res_hs20)
...
gsed = ggplot(res, aes(x=length, y=sed, fill=cat)) + theme(plot.margin=unit(c(1,0,1,1),'cm')) +
  ...
  geom_label_repel(data=focal_label, aes(label=gene), colour=blackcol, fill='white', force=2) +
  #scale_colour_manual(values=list("other"=blackcol, "focal"="#ee7700cc", "control"=violetcol)) +
  scale_fill_manual(values=list("other"=blackcol, "focal"="#ee7700cc", "control"=blackcol)) +
  scale_x_log10nice("Transcript length (nt)") +
  scale_y_continuous("Sedimentation score (SDs)", limits=c(-1,7)) + no.legend
gsed = add_marginal_x(gsed, "sed", "cat")
gesc = ggplot(res, aes(x=length, y=esc, fill=cat)) + 
  ...
  geom_point(data=focal, shape=21, stroke=0.25, fill=orangecol, size=2) +
  geom_point(data=focal_label, shape=21, stroke=0.25, size=3) +
  geom_label_repel(data=focal_label, aes(label=gene), colour=blackcol, fill="white", force=10, maxoverlaps=0) +
  scale_fill_manual(values=list("other"=blackcol, "focal"="#ee7700cc", "control"=blackcol)) +
  scale_x_log10nice("Transcript length (nt)") + 
  scale_y_continuous("Escape score (SDs)", limits=c(-2,4)) + no.legend
gesc = add_marginal_x(gesc, "esc", "cat") # = ggMarginal(gesc, margins="y", groupColour=T, type="density")
gsedesc = gsed / gesc
