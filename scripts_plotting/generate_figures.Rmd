...
# Build results table
fit_table = tibble(
  Temperature=c('30C','42C','46C'),
  beta = c(paste(format(beta_fit, digits=3)),paste(format(beta_fit, digits=3),'*'),paste(format(beta_fit, digits=3),'*')),
  chi = c(paste(format(chi_fit, digits=3)),paste(format(chi_fit, digits=3),'*'),paste(format(chi_fit, digits=3),'*')),
  mu=c("NA",paste(format(mu_fit_42C, digits=3)), paste(format(mu_fit_46C, digits=3))),
  nu=c("NA",paste(format(nu_fit_42C, digits=3, sci=F)), paste(format(nu_fit_46C, digits=3))),
...
#  L.half=c("NA",paste(round(log(2)/nu_fit_42C, digits=3)), paste(round(log(2)/nu_fit_46C, digits=3))),
# L.equiv = length at which observed nu generates same probability of interaction as observed mu
  L.equiv=c("NA",paste(round(mu_fit_42C/nu_fit_42C, digits=0)), paste(round(mu_fit_46C/nu_fit_46C, digits=0)))
)
...
p_stressSpecificEscape_heatEtOH <- ggplot(
  ...
  geom_jitter(data = df_heatEtOHStress_long %>% filter(Stress %in% c('42°C','7.5% Ethanol'), inducedCategory == "other"),
    shape = 16,height = 0,alpha = 0.05, size = 0.5) +
 geom_jitter(data = df_heatEtOHStress_long %>% filter(Stress %in% c('42°C','7.5% Ethanol'), inducedCategory != "other"),
    shape = 16,height = 0,alpha = 0.3, size = 0.5, aes(color = inducedCategory), width = 0.25) +
  ...
  scale_color_manual(values = c('orange','#A155B9','brown')) +
  facet_grid(cols = vars(Stress)) +
  theme(legend.position = 'none')
p_stressSpecificEscape_heatEtOH
...
ggsave(file.path(github_dir,"figures","Figure3/stressSpecificEscape_heatEtoh.pdf"),
       p_stressSpecificEscape_heatEtOH,
       width=70,height=50,units="mm",bg="transparent")
```

# Try azide again, this time with escape scores and top 100 transcripts.

Hmm don't really see escape of azide specific transcripts. Maybe look at a different azide data set (like 0.5%?) next.
