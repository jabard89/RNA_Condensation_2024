...
df_stats_SPFC_YONL <- df %>%
  ...
  filter(key == "SP.FC") %>%
  filter(Probe == "YONL") %>%
  group_by(Probe,UTR5,Temperature,Experiment_group,key) %>%
  nest %>%
  mutate(t_test = map(data, function(df){
    t.test(df$value ~ df$Reporter_age, 
           alternative = "two.sided"
    ) %>% broom::tidy()
  })) %>%
  select(-data) %>%
  unnest(t_test) %>%
  select(Probe, UTR5, Temperature, key, p.value) %>%
  mutate(p.format = format(p.value, digits = 2))
native_rep_comb_plot_stats <- df_stats_SPFC %>%
  filter(Probe == "YONL") %>%
  filter(Temperature != "30C") %>%
  arrange(UTR5,Temperature) %>%
  mutate(xmin = case_when(Temperature == "42C" ~ 1.75,
                          Temperature == "46C" ~ 2.75),
         xmax = case_when(Temperature == "42C" ~ 2.25,
                          Temperature == "46C" ~ 3.25)
...
<<<<<<< HEAD
  scale_y_continuous(breaks=SP.FC.breaks,labels=SP.FC.labels) #+
  # geom_signif(
  #   data = native_rep_comb_plot_stats,
  #   aes(xmin = xmin, xmax = xmax, annotations = p.format),
  #   y_position = 0.2,
  #   textsize = 6*0.35,
  #   size = 0.5,
  #   color = "black",
...
df <- df_Zsup_Poly_minfilt %>% filter(Control==FALSE,Stress_group%in%c("Azide 0.8%","Ethanol 7.5%","Heat Shock"),
  ...
  mutate(TE_cat = cut(FC.vs.ctrl_TE, 
                     breaks = quantile(FC.vs.ctrl_TE, probs = c(0, 0.25, 0.5, 0.75, 1)),
                     labels = c("0-\n25%", "25-\n50%", "50-\n75%", "75-\n100%"),
                     include.lowest = TRUE),
         TPM_cat=if_else(TPM.FC.SedSeq<=0.66,"down",
                        if_else(TPM.FC.SedSeq>=1.5,"up","other"))) %>%
  # Filter to keep only TPM_cat = "up"
  filter(TPM_cat == "up")
dge_wid <- 0.1
stress_labs <- c("42C"="42\u00B0C","Azide 0.8%"="0.8% Azide","Ethanol 7.5%"="7.5% EtOH")
p_esc_byTPMbyTE <- ggplot(df,
  ...
  scale_fill_manual(values = c("42C" = stress.cols[["42C"]],
                               "Azide 0.8%" = stress.cols[["Azide\n0.8%"]],
                               "Ethanol 7.5%" = stress.cols[["Ethanol\n7.5%"]])) +
  labs(x="Change in translation",y="",title="TPM up")+
...
plot_density_x_cat <- function(df,var,scale,min,max,fill_cat,fill_colors,alph=0.5,siz=0.25){
  X <- sym(var)
  cat_sym <- sym(fill_cat)
  density_data <- ggplot_build(ggplot(df, aes(x = !!X, fill = !!cat_sym)) + 
                            geom_density())
  max_density <- max(density_data$data[[1]]$density) * 1.1  # Add 10% buffer
  if (scale %in% c("log2","log10")){
    p <- ggplot(df,aes(x=log10(!!X),fill=!!cat_sym))+
      geom_density(alpha=alph,size=siz)+
    ...
    p <- ggplot(df,aes(x=!!X,fill=!!cat_sym))+
      geom_density(alpha=alph,size=siz)+
      scale_fill_manual(values=fill_colors)+
      theme_void()+
      guides(fill="none")+
      coord_cartesian(xlim=c(min,max), ylim = c(0, max_density), expand=F)
  }
  return(p)
}


plot_fc_xdens <- function(df,xvar,yvar,
                           xmin,xmax,ymin,ymax,
                          yvar_cutoff,
                         cat,cat_to_highlight,cat_colors,
                           xscale,xbreaks,yscale,ybreaks) {
  X <- sym(xvar)
  Y <- sym(yvar)
  cat_sym <- sym(cat)
  ...
  p_scatter <- ggplot(df,
    ...
    geom_vline(xintercept = xint,
               linetype = 'dashed') +
    ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(!!Y < yvar_cutoff | !!cat_sym %!in% cat_to_highlight),alpha=0.4,shape=16,size=1, color = "grey50")) +
    ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(!!Y >= yvar_cutoff & !!cat_sym %in% cat_to_highlight), aes(color = !!cat_sym), alpha=0.6,shape=16,size=2)) +
    guides(color="none")+
  ...
  p_dens_x <- plot_density_x_cat(df %>% filter(!!Y >= yvar_cutoff & !!cat_sym %in% cat_to_highlight),xvar,xscale,xmin,xmax,cat,cat_colors)
  # Overlay the density plots onto the main plot using annotation_custom
  combined_plot <- egg::ggarrange(
    p_dens_x, p_scatter,
    ncol = 1,
    heights = c(0.3, 1)  # Density plot takes 30% of the vertical space
  )
  return(combined_plot)
}

p_HS <- plot_fc_xdens(df = df %>% filter(Stress_label == "42C"), xvar = "esc.mean", yvar = "TPM.FC.SedSeq", 
                      xmin = (-3),xmax = 5,ymin = 1, ymax = 100, yvar_cutoff = 1.5,
                      "TE_cat", c("0-\n25%","75-\n100%"), c("0-\n25%" = "#4575b4", "75-\n100%" = "#d73027"),
                      xscale="linear",xbreaks=c(-2,0,2),yscale="log10",ybreaks=c(1,10,100))
p_HS


p_EtOH <- plot_fc_xdens(df = df %>% filter(Stress_label == "Ethanol 7.5%"), xvar = "esc.mean", yvar = "TPM.FC.SedSeq", 
                      xmin = (-3),xmax = 5,ymin = 1, ymax = 100, yvar_cutoff = 1.5,
                      "TE_cat", c("0-\n25%","75-\n100%"), c("0-\n25%" = "#4575b4", "75-\n100%" = "#d73027"),
                      xscale="linear",xbreaks=c(-2,0,2),yscale="log10",ybreaks=c(1,10,100))
p_EtOH

p_N3 <- plot_fc_xdens(df = df %>% filter(Stress_label == "Azide 0.8%"), xvar = "esc.mean", yvar = "TPM.FC.SedSeq", 
                      xmin = (-3),xmax = 5,ymin = 1, ymax = 100, yvar_cutoff = 1.5,
                      "TE_cat", c("0-\n25%","75-\n100%"), c("0-\n25%" = "#4575b4", "75-\n100%" = "#d73027"),
                      xscale="linear",xbreaks=c(-2,0,2),yscale="log10",ybreaks=c(1,10,100))
p_N3
...
p_TPMFC_esc <- ggplot(df,
       aes(x=TPM.FC.SedSeq,y=esc.mean,color=TE_cat))+
  geom_hline(yintercept=0,linetype="dotted")+geom_vline(xintercept=1,linetype="dotted")+
  ggrastr::rasterize(dpi = 600, geom_point(data = df %>% filter(TE_cat %in% c("25-\n50%","50-\n75%")),
                                           shape=16,alpha=0.3, color = "grey50")) +
  ggrastr::rasterize(dpi = 600, geom_point(data = df %>% filter(TE_cat == "0-\n25%"),
                                           shape=16,alpha=0.3)) +
  ggrastr::rasterize(dpi = 600, geom_point(data = df %>% filter(TE_cat == "75-\n100%"),
                                           shape=16,alpha=0.3)) +
  scale_x_log10nice(omag=c(-1,0,2,4))+
  scale_y_continuous(breaks=c(-3,0,3))+
  coord_cartesian(xlim=c(1,50),ylim=c(-4.3,4.3),expand=F)+
  facet_grid(.~Stress_label)+
  labs(x="",y="")+
  theme(panel.spacing.y = unit(1.1,"mm"))
p_TPMFC_esc
...
df_30C_occ <- df_Occ_mean_minfilt %>%
  ...
  summarise(Occ.odds.mean = exp(mean(log(Occ.odds.mean))))
df <- df_hairpins %>% full_join(df_30C_occ,by="ORF") %>%
  filter(!is.na(newZ.mean),!is.na(Occ.odds.mean),!is.infinite(Occ.odds.mean),Occ.odds.mean>0) %>%
  left_join(df_Weinberg,by="ORF") %>%
  filter(Occ.odds.mean>0) %>%
  left_join(df_rnafold %>%
              select(ORF,EnergyBin),by="ORF") %>%
...
energy_color_scale <- c("least" = RColorBrewer::brewer.pal(12,"Paired")[4], 
                              "other" = "grey50", 
                              "most" = RColorBrewer::brewer.pal(12,"Paired")[10])
p_Occ_newZ <- ggplot(df,
       aes(x=Occ.odds.mean,y=newZ.mean))+
  geom_hline(yintercept=0,linetype="dotted")+
  geom_vline(xintercept=median(df$Occ.odds.mean,na.rm=T),linetype="dotted")+
  ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(EnergyBin=="other"),alpha=0.5, shape = 16, size = 0.8, color = "grey")) +
  ggrastr::rasterize(dpi = 600, geom_point(data=df%>%filter(EnergyBin!="other"),alpha=0.75,aes(color=EnergyBin), shape = 16, size = 0.8)) +
  ...
  scale_x_log10nice()+
  geom_text(data=df_corr %>% filter(term=="Occ.odds.mean"),
             aes(label=paste0("r = ",round(newZ.mean,digits=2)),
                 x=1,y=2.8),
            size=10*0.35)+
  geom_smooth(method="lm",se=F,color="black")+
  #geom_point(data=df %>% filter(gene%in%genes_to_label),size=2,color="red")+
  ...
  theme(legend.position="none")
p_Occ_newZ
p_newZ_box <- ggplot(df %>% filter(!is.na(EnergyBin)),
         aes(x=EnergyBin,y=newZ.mean,fill=EnergyBin))+
    ggrastr::rasterize(dpi = 600, geom_point(position=position_jitterdodge(jitter.width=1.5),
               aes(color=EnergyBin,alpha=EnergyBin), shape =16, size = 0.75)) +
  scale_color_manual(values=energy_color_scale)+
    scale_alpha_manual(values=c("least" = 0.6, "other" = 0.1, "most" = 0.6)) +
  ...
  theme(axis.text.x = element_text(angle=45,hjust=1))
p_newZ_box
ggsave("../figures/Figure5/newZ_Occ_left.pdf",
       egg::set_panel_size(p=p_Occ_newZ,width=unit(30,"mm"),height=unit(30,"mm")),
       width=75,height=75,units="mm",bg="transparent")
ggsave("../figures/Figure5/newZ_Occ_right.pdf",
       egg::set_panel_size(p=p_newZ_box+theme(axis.title.y=element_blank()),width=unit(30,"mm"),height=unit(30,"mm")),
       width=75,height=75,units="mm",bg="transparent")
...
ggsave("../figures/Figure5/TPM_TE_left.pdf",
       egg::set_panel_size(p=p_TPM_TE+labs(x="",y=""),width=unit(30,"mm"),height=unit(30,"mm")),
       width=100,height=100,units="mm",bg="transparent")
ggsave("../figures/Figure5/TPM_TE_newZ_right.pdf",egg::set_panel_size(p=p_newZ_box+labs(x="",y=""),width=unit(15,"mm"),height=unit(30,"mm")),
       width=100,height=100,units="mm",bg="transparent")
```
...
df <- read_tsv(file.path(github_dir,"data_processed/sedseq_out.tsv.gz")) %>%
  ...
  select(ORF,Strain,Treatment_group,Treatment,Treated,Temperature,newZ) %>%
  group_by(ORF,Strain) %>%
  summarise(n = length(newZ),
            newZ.se = sd(newZ)/sqrt(length(newZ)),
            newZ.mean = mean(newZ)) %>%
...
p_reps <- ggplot(df %>%arrange(desc(TE_norm)),
       aes(x=TE_norm,y=newZ.mean))+
  geom_errorbar(aes(x=TE_norm,ymin=newZ.mean-newZ.se,ymax=newZ.mean+newZ.se),width=0.5,size=0.5)+
...
p_dens <- ggplot(df_dist,aes(x=newZ.mean))+
...
p_Zsup_corr_nocolor <- ggplot(df,
       aes(x=Occ.odds.mean,y=newZ.mean,color=Occ_bin))+
...
df_nocompress <- df_sedseq_withreps %>%
  select(ORF,Strain,Treatment_group,Treatment_group_ID,Stress,Stress_group,Stress_label,Treatment,Temperature,Biorep,Control,newZ) %>%
  pivot_wider(names_from=Biorep,values_from=newZ) %>%
  filter(Stress_group %in% c("Heat Shock","Azide 0.5%","Azide 0.8%",
                             "Ethanol 5%","Ethanol 7.5%","Ethanol 10%","Ethanol 15%")) %>%
...
df_compress <- df_sedseq_withreps %>%
  select(ORF,Strain,Treatment_group,Treatment_group_ID,Stress,Stress_group,Stress_label,Treatment,Temperature,Biorep,Control,newZ) %>%
  mutate(newZ.compress = if_else(newZ>maxZ,maxZ,
                                   if_else(newZ<minZ,minZ,newZ))) %>%
  select(-newZ) %>%
  pivot_wider(names_from=Biorep,values_from=newZ.compress) %>%
  filter(Stress_group %in% c("Heat Shock","Azide 0.5%","Azide 0.8%",
                             "Ethanol 5%","Ethanol 7.5%","Ethanol 10%","Ethanol 15%")) %>%
  filter(Temperature!="42CR", Control == TRUE) %>%
  arrange(Stress,Treatment_group_ID,Stress_label) %>%
...
p_out_HS_newZ <- plot_table %>% filter(Stress=="Heat Shock", Treatment_group_ID == "tg_1") %>% pull(plot) %>% .[[1]]
p_out_HS_newZ
p_out_N3_tg20_newZ <- plot_table %>% filter(Stress=="Azide",Treatment_group_ID=="tg_20") %>% pull(plot) %>% .[[1]]
p_out_N3_tg20_newZ
p_out_N3_tg9_newZ <- plot_table %>% filter(Stress=="Azide",Treatment_group_ID=="tg_9") %>% pull(plot) %>% .[[1]]
p_out_N3_tg9_newZ
p_out_EtOH_tg10_newZ <- plot_table %>% filter(Stress=="Ethanol",Treatment_group_ID=="tg_10") %>% pull(plot) %>% .[[1]]
p_out_EtOH_tg10_newZ
p_out_EtOH_tg11_newZ <- plot_table %>% filter(Stress=="Ethanol",Treatment_group_ID=="tg_11") %>% pull(plot) %>% .[[1]]
p_out_EtOH_tg11_newZ
p_out_EtOH_tg12_newZ <- plot_table %>% filter(Stress=="Ethanol",Treatment_group_ID=="tg_12") %>% pull(plot) %>% .[[1]]
p_out_EtOH_tg12_newZ

ggsave("../figures/Supplemental/compare_newZ_bioreps_HS.pdf",
       egg::set_panel_size(p=p_out_HS_newZ,width=unit(15,"mm"),height=unit(15,"mm")),
       width=100,height=50,units="mm",bg="transparent")
ggsave("../figures/Supplemental/compare_newZ_bioreps_N3_tg20.pdf",
       egg::set_panel_size(p=p_out_N3_tg20_newZ,width=unit(15,"mm"),height=unit(15,"mm")),
       width=100,height=50,units="mm",bg="transparent")
ggsave("../figures/Supplemental/compare_newZ_bioreps_N3_tg9.pdf",
       egg::set_panel_size(p=p_out_N3_tg9_newZ,width=unit(15,"mm"),height=unit(15,"mm")),
       width=100,height=50,units="mm",bg="transparent")
ggsave("../figures/Supplemental/compare_newZ_bioreps_EtOH_tg10.pdf",
       egg::set_panel_size(p=p_out_EtOH_tg10_newZ,width=unit(15,"mm"),height=unit(15,"mm")),
       width=100,height=50,units="mm",bg="transparent")
ggsave("../figures/Supplemental/compare_newZ_bioreps_EtOH_tg11.pdf",
       egg::set_panel_size(p=p_out_EtOH_tg11_newZ,width=unit(15,"mm"),height=unit(15,"mm")),
       width=100,height=50,units="mm",bg="transparent")
ggsave("../figures/Supplemental/compare_newZ_bioreps_EtOH_tg12.pdf",
       egg::set_panel_size(p=p_out_EtOH_tg12_newZ,width=unit(15,"mm"),height=unit(15,"mm")),
       width=100,height=50,units="mm",bg="transparent")
```
...
df <- df_Zsup_mean %>% filter(Treatment_group=="EW_TSPP") %>%
  ...
  mutate(target = if_else(label=="HSF1","HSF1 targets",
                          if_else(label == "MSN2/4","MSN2/4 targets","other"))) %>%
  mutate(target=factor(target,levels=c("other","MSN2/4 targets","HSF1 targets"))) %>%
  arrange(target) %>%
  ungroup %>%
