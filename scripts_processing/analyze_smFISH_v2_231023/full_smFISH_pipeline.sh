...
#                 echo "${cond_dir}"
#                 #echo "Running extract most in focus"
#                 mamba run -n ${BIGFISH_ENV} python ${extract_most_infocus_prog} ${DATE} ${TRANSCRIPT} ${CONDITION} ${SRC_DIR} ${CHANNEL_LIST} ${PAB1_CHAN} ${FOCUS_BUFFER}
#                 echo "Running make mip"
#                 mamba run -n ${BIGFISH_ENV} python ${bigfish_make_mip_prog} ${DATE} ${TRANSCRIPT} ${CONDITION} ${SRC_DIR} "647,A488" --filt_focus
#             fi
#         done
#     done
# done
...
for DATE in "${DATES[@]}"; do
    for TRANSCRIPT in "${TRANSCRIPTS[@]}"; do
        for CONDITION in "${CONDITIONS[@]}"; do
            ...
            if [ -d "${cond_dir}/stacks" ]; then
                echo "${cond_dir}"
                #echo "Running test threshold"
                #mamba run -n ${BIGFISH_ENV} python ${bigfish_test_thresh_prog} ${DATE} ${TRANSCRIPT} ${CONDITION} ${SRC_DIR} ${RNA_CHAN} ${VOX_X} ${VOX_Y} ${VOX_Z} ${THRESH} --filt_focus
                #echo "Running run bigfish"
                mamba run -n ${BIGFISH_ENV} python ${bigfish_run_all_prog} ${DATE} ${TRANSCRIPT} ${CONDITION} ${SRC_DIR} ${RNA_CHAN} ${VOX_X} ${VOX_Y} ${VOX_Z} ${THRESH} --filt_focus
                mamba run -n ${ANALYSIS_ENV} python ${bigfish_analyze_prog} ${DATE} ${TRANSCRIPT} ${CONDITION} ${SRC_DIR} ${RNA_CHAN} ${PAB1_CHAN} ${THRESH} ${RANDPOINTS} ${RNA_BUFFER} --filt_focus
            fi
        done
    done
done
